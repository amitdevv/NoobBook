...
# =============================================================================
# Direct-to-Supabase signed uploads
# =============================================================================
#
# The browser POSTs through nginx → Flask → Supabase for normal uploads, which
# means the entire file body has to traverse the edge proxy (Cloudflare on the
# user's deployment) AND Flask. Cloudflare Free caps body bytes at 100 MB so
# anything bigger 413s before reaching us.
#
# Direct-upload solves both problems: the backend mints a short-lived signed
# URL pointing at Supabase Storage, the browser PUTs the file straight to that
# URL (different hostname, not behind our edge proxy or Flask), and only a
# tiny "I'm done" call comes back through normal channels for source-row
# creation. Supabase's own 1 GB FILE_SIZE_LIMIT becomes the real cap.

def _public_supabase_url() -> Optional[str]:
    """
    Return the externally-reachable Supabase URL, if configured.

    In a Coolify / Docker setup ``SUPABASE_URL`` typically points at the
    internal kong hostname (``http://supabase-kong:8000``) which the
    browser can't resolve. ``SUPABASE_PUBLIC_URL`` carries the public
    hostname for client-facing URLs. Returning ``None`` means the
    SUPABASE_URL is already public (e.g. ``https://xxxx.supabase.co``)
    and no rewrite is needed.
    """
    public = (os.getenv("SUPABASE_PUBLIC_URL") or "").strip().rstrip("/")
    return public or None


def _rewrite_to_public(url: str) -> str:
    """
    Swap the host of a Supabase-issued URL for the public-facing host.

    Supabase's signed-upload-URL response is built from the client's
    configured base URL, which inside the container is the internal
    kong hostname. We rewrite to ``SUPABASE_PUBLIC_URL`` so the browser
    can actually reach it. No-op if SUPABASE_PUBLIC_URL is unset or the
    URL is already on the public host.
    """
    public = _public_supabase_url()
    if not public:
        return url
    try:
        parsed = urlparse(url)
        ...
        logger.warning("Failed to rewrite signed URL to public host: %s", exc)
        return url
...
def create_signed_upload_url(
    ...
    """
    Mint a single-use signed URL the browser can PUT a file to.

    The returned ``upload_url`` already carries the upload token in its
    query string and points at the public Supabase hostname. The browser
    sends the file body as the PUT request body (Content-Type matters —
    Supabase records it as the object's stored mime).

    Args:
        project_id: Project UUID — used as the path prefix.
        source_id:  Source UUID generated by the caller.
        filename:   Storage filename (typically ``{source_id}{ext}``).

    Returns:
        ``{"upload_url", "path", "token"}`` on success, ``None`` on
        failure (the underlying error is logged).
    """
    client = _get_client()
    path = _build_path(project_id, source_id, filename)

    try:
        # Supabase-py returns ``{"signed_url", "path", "token"}`` for
        # this call. The signed_url already has the token embedded.
        resp = client.storage.from_(BUCKET_RAW).create_signed_upload_url(path)
    except Exception as exc:
        logger.error("Failed to create signed upload URL for %s: %s", path, exc)
        return None

    # Normalize the response across supabase-py versions (some return
    # camelCase, some snake_case).
    signed_url = (
        (resp.get("signed_url") if isinstance(resp, dict) else None)
        or (resp.get("signedUrl") if isinstance(resp, dict) else None)
    )
    token = resp.get("token") if isinstance(resp, dict) else None

    if not signed_url:
        logger.error("create_signed_upload_url returned no URL for %s: %s", path, resp)
        return None

    return {
        "upload_url": _rewrite_to_public(signed_url),
        "path": path,
        "token": token,
    }


def raw_object_exists(project_id: str, source_id: str, filename: str) -> bool:
    """
    Confirm a file actually landed in the raw-files bucket.

    Used by the upload-finalize endpoint as a sanity check before we
    create the source row — prevents an attacker from finalizing a row
    they never uploaded for, and surfaces silent client-side failures
    (e.g. browser closed mid-upload) as a clean 400 instead of a
    "ready" source row pointing at nothing.
    """
    client = _get_client()
    folder = f"{project_id}/{source_id}"
    try:
        entries = client.storage.from_(BUCKET_RAW).list(folder, options=_LIST_OPTIONS)
        return any(e.get("name") == filename for e in (entries or []))
    except Exception as exc:
        logger.warning("raw_object_exists check failed for %s/%s: %s", folder, filename, exc)
        return False
